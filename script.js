document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- INITIAL LOADING ANIMATION (FIXED DARK SCREEN) ---
  function revealSite() {
  ...
  // Force reveal immediately after DOM matches content
  revealSite();
  ...
  function openModal(title, desc, imgString, skillsString, pdfLink, modelLink, videoLink, hasChart, codeSnippet, cardId) {
      ...
      modal.style.display = 'flex'; // Fix: Use flex to center with CSS
  }
  ...
  // --- CONTACT FORM HANDLER (ROCKET ANIMATION) ---
  ...
  // --- VISIBLE COMET CLASS (FIXED VISIBILITY) ---
  class Comet {
    constructor() { this.reset(); }
    reset() { 
      const side = Math.floor(Math.random() * 4); const buffer = 100; // Small buffer for quick entry
      ...
      this.radius = Math.random() * 3 + 4; // Larger 4-7px
      this.vx = (Math.random() - 0.5) * 8; // Faster
    ...
    draw() { 
        ...
        // CONE TRAIL EFFECT (Not just circles)
        if (!isBlueprint && this.history.length > 1) { 
             for(let i = 0; i < this.history.length; i++) {
                 const point = this.history[i];
                 const size = (i / this.history.length) * this.radius; // Cone shape
  ...
  window.addEventListener('resize', () => { resize(); initSpace(); });
  resize(); initSpace(); animate();
});